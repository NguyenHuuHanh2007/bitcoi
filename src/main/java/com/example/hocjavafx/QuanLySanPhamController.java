...
public class QuanLySanPhamController implements Initializable {
    ...
    public void themSanPham(ActionEvent event) {
        try {
            ...
            // 1. CHẶN LỖI ĐỂ TRỐNG (Đúng phong cách của bạn)
            if (ten.isEmpty() || soLuongInput.isEmpty()) {
                throw new IllegalArgumentException("Vui lòng nhập đầy đủ Tên sản phẩm và Số lượng!");
            }

            // 2. CHẶN LỖI SAI ĐỊNH DẠNG SỐ VÀ SỐ ÂM
            int sl;
            try {
                sl = Integer.parseInt(soLuongInput);
            } catch (NumberFormatException e) {
                // Đổi lỗi từ hệ thống sang lỗi dễ hiểu cho người dùng
                throw new IllegalArgumentException("Số lượng phải là một con số hợp lệ (VD: 10, 50)!");
            }

            if (sl <= 0) {
                throw new IllegalArgumentException("Số lượng sản phẩm thêm mới phải lớn hơn 0!");
            }

            // 3. XỬ LÝ DỮ LIỆU MẶC ĐỊNH
            if (tinhTrang.isEmpty()) tinhTrang = "Chưa cập nhật";
            ...
            // Hiển thị chính xác thông báo lỗi mà chúng ta đã "ném" (throw) ở trên
            new Alert(Alert.AlertType.ERROR, e.getMessage()).showAndWait();
        } catch (Exception e) {
            // Bắt các lỗi không xác định khác (phòng ngừa rủi ro)
            new Alert(Alert.AlertType.ERROR, "Đã xảy ra lỗi hệ thống: " + e.getMessage()).showAndWait();
        }
    }
