...
public class PhongLiveController implements Initializable {
    ...
    public void guiBinhLuan(ActionEvent event) {
        ...
        if (!tinNhan.isEmpty()) {
            txtChatLive.appendText("[" + thoiGianHienTai() + "] " + Session.tenNguoiDung + " (Bạn): " + tinNhan + "\n");
    ...
    public void thaTim(ActionEvent event) {
        txtChatLive.appendText("[" + thoiGianHienTai() + "] " + Session.tenNguoiDung + " (Bạn) đã thả tim ❤️❤️❤️\n");
    }
    ...
    public void raGiaLive(ActionEvent event) {
        ...
        try {
            ...
            // Kiểm tra ví tiền (Liên kết chặt chẽ với Session)
            if (giaTra > Session.soDu) {
                thongBaoLoi("Tài khoản không đủ! Bạn chỉ còn " + String.format("%,.0f VNĐ", Session.soDu) + ". Vui lòng nạp thêm.");
                return;
            }

            // Đủ tiền -> Xử lý giao dịch
            Session.soDu -= giaTra;
            Session.lichSuGiaoDich.add("[-] Đấu giá Live thành công - Trừ: " + String.format("%,.0f VNĐ", giaTra));

            // Xóa ô nhập liệu
            txtNhapLieu.clear();

            // Phát thông báo chấn động lên khung chat Live
            txtChatLive.appendText("\n🔥🔥🔥 [" + thoiGianHienTai() + "] ĐẠI GIA " + Session.tenNguoiDung.toUpperCase() + " ĐÃ RA GIÁ KỶ LỤC: " + String.format("%,.0f VNĐ", giaTra) + " !!! 🔥🔥🔥\n\n");
