...
public class TrangChuController implements Initializable {
    ...
    // --- HÀM CHI TIẾT SẢN PHẨM (GIÚP HẾT LỖI LOAD FXML) ---
    @FXML
    ...
    // --- HÀM ĐẤU GIÁ: TÍNH TỔNG TIỀN = GIÁ X SỐ LƯỢNG ---
    @FXML
    public void thamGiaDauGia(ActionEvent event) {
        ...
        if (result.isPresent()) {
            try {
                ...
                if (tongTienPhaiTra > Session.soDu) {
                ...
                if (confirm.showAndWait().get() == ButtonType.OK) {
                    Session.soDu -= tongTienPhaiTra;
                    phien.setGiaHienTai(giaMoiMoiMon);
                    Session.lichSuGiaoDich.add("[-] Đấu giá: " + phien.getTenSanPham() + " (x" + soLuong + ") - Trừ: " + String.format("%,.0f VNĐ", tongTienPhaiTra));
                    capNhatHienThiUser();
                    tableAuctions.refresh();
                }
    ...
    public void capNhatHienThiUser() {
        if (lblUsername != null) {
            lblUsername.setText("Xin chào, " + Session.tenNguoiDung + " | Số dư: " + String.format("%,.0f VNĐ", Session.soDu));
        }
    }
